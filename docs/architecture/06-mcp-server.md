...
| `search_memory` | Most-relevant memories. Hybrid only when a semantic embedder is active, else the static keyword surface (parent FTS plus bounded Gmail message-segment FTS) | `query`*, `scope`, `limit`, `source`, `since_hours` | `limit` = `mcpSearchDefaultLimit` = **8**. Bodies snippeted to 240 runes. Payload is `{results, freshness, health}`, plus `filters`/`excluded_by_filter` when a source/since_hours filter is applied (#241 below). A Gmail thread result with a matching derived message segment carries `evidence: {evidence_ref, sender, at, snippet}` naming its strongest matching message (#243) |
