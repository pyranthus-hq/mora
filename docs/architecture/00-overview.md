...
| `internal/ingest` | `internal/atomicio`, `internal/config`, `internal/memory` | Connector-ingest journal and live-run lease lifecycle: guards/paths/durable append/status, PID ownership/reclamation, and post-rebuild recovery/compaction with covered/gone path retirement and live-lease protection. Stage B extraction; connector dispatch and registry wiring remain in `internal/mora`. |
