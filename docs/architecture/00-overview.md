...
| `internal/ingest` | `internal/atomicio`, `internal/config`, `internal/memory` | Connector-ingest journal and live-run lease primitives: state-root guard, keys/paths, durable append, fail-closed status, validated headers, PID ownership, dead-owner reclamation, and scoped/all-owned release. Stage B extraction; journal recovery/compaction, connector dispatch, and registry wiring remain in `internal/mora`. |
