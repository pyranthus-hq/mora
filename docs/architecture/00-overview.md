...
| `internal/digest` | `internal/genericutil` | Deterministic Markdown presentation for digest headers, health/freshness framing, urgent shelves, source sections, nested commitment citations, and structural item/section budgeting with exact survivor IDs. Mora retains vault/source loading, health snapshot construction, delta/watermark policy, ranking, commitment attachment, CLI/MCP orchestration, and final publication. |
