...
- **`read_memory`** (`mcpReadMemory`, `mcp.go`): `findMemory(cfg, id)` — full body and metadata, with no snippeting. This is how agents expand a preview returned by `search_memory` or `list_memory`.
