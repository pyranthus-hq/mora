...
| `internal/search` | `internal/memory` | Safe FTS5 query/CLI parsing, trusted-source/time filters, match-centered previews, and deterministic corroborating-record clustering with legacy slot/backfill discipline and refusal caps. Stage B extraction; catalog validation, retrieval execution, fusion, later-related annotation, and context assembly remain in `internal/mora`. |
