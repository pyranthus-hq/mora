...
| `internal/commitment` | standard library | Provider-neutral speech ownership/manual-promise adjudication plus shared obligation direction/state/due vocabulary, deadline classification, stable evidence identity, deterministic guarded lifecycle transitions, ambiguity gaps, and provenance-gated duplicate/canonical projections. Mora retains provider-specific actor/party/evidence extraction, citation construction/merging, health-aware materialization, SQLite generation ownership, and governance application. |
...
| `internal/meeting` | `internal/commitment`, `internal/governance`, `internal/graph`, `internal/health`, `internal/memory` | Validated cited meeting-brief DTOs/correction commands/historical rails, deterministic rendering/section assembly/ambiguous-attribution resolution, plus deterministic next-event selection, sender-authored evidence text/historical framing, participant-roster gating, and pure meeting-evidence classification: notification/third-party/service exclusion, provider-specific ownership, actionable-question boundaries, unresolved/staleness/material-context policy. Mora supplies identity/config/service/time facts and retains attribution, governance decisions, vault/graph loading, ranking, budgeting, health-banner derivation, and CLI/MCP assembly. |
