...
| `internal/mcp` | (leaf — no internal deps) | MCP newline-delimited JSON-RPC framing and spec-compliant tool-result envelopes: request caps, malformed/notification suppression, ordered response frames, text payloads, object structured content, and tool errors. Initialization, registry/dispatch, governance, and handlers remain in `internal/mora`. |
