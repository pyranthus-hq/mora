...
| `internal/ingest` | `internal/atomicio`, `internal/config`, `internal/memory` | Connector-ingest journal identity and observation primitives: absolute state-root guard, safe per-instance keys/paths, durable line append, fail-closed scanning/status, and validated run-header extraction. Stage B extraction; leases, journal recovery/compaction, connector dispatch, and registry wiring remain in `internal/mora`. |
