...
| `internal/search` | `internal/config`, `internal/genericutil`, `internal/memory` | Query/CLI parsing, trusted-source/time filters, previews, corroboration clustering, conservative JSON result budgeting, and deterministic control-file/memory context assembly. Stage B extraction; catalog validation, retrieval execution/fusion, later-related annotation, and CLI/MCP wiring remain in `internal/mora`. |
