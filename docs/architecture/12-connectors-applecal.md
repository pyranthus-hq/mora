...
- **No `source_created_at`.** Google Calendar records one from `Event.Created` (see [google](./04-connectors-google.md)), so the browse row's [creation clock](./01-data-model-and-storage.md#the-three-clocks-on-a-memory-218) can be shown for a Google event and not an Apple one. The store's `creation_date` describes when the LOCAL replica's row was written by a sync, not when the event was created at its origin — a fresh Mac or a re-added account restamps every event — so it would read as a creation time while meaning "when this Mac last resynced". The field is left off rather than filled with a clock that answers a different question. Wiring a trustworthy one needs a connector-side signal Apple's store does not currently expose.
