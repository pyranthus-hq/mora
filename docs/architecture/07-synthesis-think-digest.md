...
**Every production surface is preview by default.** The MCP `digest` tool has no `advance` arg at all; `pulse --digest` and `pulse --write --digest` are both preview; the *only* committer is the scheduled `pulse --write --digest --advance --sync --brief-file --notify` (`scheduleCommands["pulse-daily"]`). See [sync-and-freshness](./11-sync-and-freshness.md) for the launchd `RunAtLoad` drop that stops a reboot from consuming the morning delta.
...
- **`--advance` is the ONLY committer; everything else is preview (SC#4/D-02).** The MCP `digest` tool has no `advance` arg (`mcp.go`); the MCP `brief` tool has none either (`mcp.go`, preview by construction); only the scheduled `pulse --write --digest --advance --sync --brief-file --notify` advances (`scheduleCommands["pulse-daily"]`, `mora.go:3441-3442`). `nextSnapshot` is the silent-data-loss guard — it keeps the OLD hash for unshown ids (so an unshown `updated` re-surfaces), advances only over shown ids, and drops deleted ids (`digest.go:360-383`). Commit holds the `O_EXCL` brief lock (`brief.go:241`, T-12-07).
