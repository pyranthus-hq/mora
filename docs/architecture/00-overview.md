...
| `internal/previewfilter` | `internal/graph`, `internal/memory` | Pure entity/scope/since-days preview filtering, alias membership sets, negative-window clamping, and fail-closed timestamp matching. Mora retains DB-backed entity resolution, user-facing ambiguity/busy errors, option parsing, and digest/MCP orchestration. |
