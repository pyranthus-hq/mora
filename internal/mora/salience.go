...
func metaMessageCount(m Memory) int {
	if m.Meta == nil {
		return 1
	}
...
// aggregatePersonSalience is the SHARED seam: it computes a person-id → salience_micros
// map from a slice of memories, using the SAME scoreSalience kernel that buildGraph
// (14-03) and the digest (14-05) both call, so they never diverge.
//
// Two passes, both deterministic:
//  1. vaultMax = the max validFromOf across all non-tombstoned memories (the recency
//     anchor — vault-relative, not wall-clock).
//  2. For each non-tombstoned memory: resolve participants via personRefs (sorted,
//     deduped), compute fanout = 1/sqrt(max(1,len(parts))), read the real message count
//     via metaMessageCount (fallback 1), and for EACH participant accrue
//     perChannelVolume[m.Type] += msgCount·fanout, record the channel, and keep the
//     latest validFrom as that person's lastSeen.
//
// Each id's kind is classified ONCE via classifyIdentity on the address alone (the
// deterministic floor). The graph (14-03) may refine the gate with a trusted display
// name, but both share THIS scoring function so the digest and graph agree. Tombstoned
// memories (DeletedAt != "") are skipped to mirror buildGraph's live-stats rule.
func aggregatePersonSalience(mems []Memory) map[string]int64 {
	...
	inputs := map[string]*salienceInput{}
	for _, m := range mems {
		if m.DeletedAt != "" {
			continue
		}
		...
		weighted := float64(metaMessageCount(m)) * fanout
		vf := validFromOf(m)
		for _, p := range parts {
			in, ok := inputs[p.id]
			if !ok {
	...
	for id, in := range inputs {
		out[id] = scoreSalience(*in, vaultMax)
	}
	return out
}
