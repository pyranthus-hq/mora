...
// on the same `.lock`: an os.ReadFile of the lock body can collide with another
...
// renameReplaceRetryable is the rename-specific spelling used by atomicWrite. On
// Windows os.Rename is MoveFileEx(MOVEFILE_REPLACE_EXISTING), which must delete
// the existing target; racing calls return the same two errnos, so the
// classification is identical to sharingViolationRetryable.
