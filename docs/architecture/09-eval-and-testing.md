...
**The ratchet is now partly green, by measurement.** Meeting Owner and Direction are typed; the validated v2 meeting row scores both at precision/recall 1.0 with `CriticalDirection == 0` and both direction classes exercised. Meeting Lifecycle and ClosureLinkage are now directly expressed with precision 1.0, and `ClosedLeaks` is zero on both frozen scorecards. Their recall remains intentionally partial on the v1/v2 corpora because those pre-PR1 Markdown fixtures do not preserve immutable message/block ids for every hidden inventory item; the adapter refuses to fabricate identity merely to improve a score. Due time, commitment identity, dedup, and citation roles remain fail-closed until their product lanes can be measured end to end. `Due` and `ClosureRef` carry explicit `none` sentinels for the gold values — without them, a surface that cannot express a field at all would score *correct* against an obligation whose gold value is none, and the row would be accidentally green. `Prediction.CommitmentID`, `DuplicateOf`, and typed citations (`opener`, `closure`, `supporting`) keep canonical selection, duplicate-copy citation retention, and closure evidence independently observable.
