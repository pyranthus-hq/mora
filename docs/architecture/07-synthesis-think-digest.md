...
- **Plain-window mode** (`opts.sinceHours > 0`, SC#2): `buildWindowDigest` considers non-deleted memories created within the requested window, then applies the shared commitment eligibility/relevance gate before grouping, ordering, and capping. It **never advances the watermark** — `cmdPulse` forces `advance=false` whenever `--since-hours>0`, even if `--advance` is also passed.
