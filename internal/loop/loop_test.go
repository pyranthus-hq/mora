...
type loopRunStatus = RunStatus
type loopRunRecord = RunRecord
type loopRegistration = Registration
type loopHealth = Health

const (
	loopRunRunning   = RunRunning
	loopRunSucceeded = RunSucceeded
	loopRunFailed    = RunFailed
)

const loopSkipExitCode = SkipExitCode
...
func loopLatestPath(cfg Config, id string) string               { return LatestPath(cfg, id) }
func loopLockPath(cfg Config, id string) string                 { return LockPath(cfg, id) }
func validLoopID(id string) bool                                { return ValidID(id) }
func periodFor(cadence string, now time.Time) string            { return PeriodFor(cadence, now) }
func loadRunRecord(cfg Config, id string) (loopRunRecord, bool) { return LoadRunRecord(cfg, id) }
func classifyLoopHealth(reg loopRegistration, rec loopRunRecord, ok bool, now time.Time) loopHealth {
	return ClassifyHealth(reg, rec, ok, now)
}
func loopBegin(cfg Config, id string, jsonOut bool, now time.Time, stdout io.Writer) error {
	return Begin(cfg, id, jsonOut, now, stdout)
}
func loopDone(cfg Config, id, runID string, ok bool, reason string, now time.Time, stdout io.Writer) error {
	return Done(cfg, id, runID, ok, reason, now, stdout)
}
func heartbeatLoopRun(cfg Config, id, runID string, now time.Time) error {
	return HeartbeatRun(cfg, id, runID, now)
}
func withLoopRunEffect(cfg Config, id, runID string, fn func() error) error {
	return WithRunEffect(cfg, id, runID, loopClock, fn)
}
func withLoopRunEffectAt(cfg Config, id, runID string, effectNow time.Time, fn func() error) error {
	return WithRunEffectAt(cfg, id, runID, effectNow, loopClock, fn)
}
func loopStatus(cfg Config, id string, jsonOut bool, now time.Time, stdout io.Writer) error {
	return Status(cfg, id, jsonOut, now, stdout)
}
