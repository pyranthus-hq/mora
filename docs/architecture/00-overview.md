...
| `internal/search` | `internal/memory` | Safe FTS5 query/CLI parsing, shared trusted-source/time-window filters, and deterministic match-centered search preview generation with metadata stripping. Stage B extraction; catalog validation, retrieval execution, fusion, and context assembly remain in `internal/mora`. |
