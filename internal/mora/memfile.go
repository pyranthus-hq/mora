...
// writeWikiIndex refreshes a derived state-cache page. It intentionally never
// writes vault/index.md: every Markdown file in the vault is user/source data.
func writeWikiIndex(cfg Config, count int, updated string) error {
	...
	return atomicio.Write(filepath.Join(cfg.StateDir, "index.md"), []byte(body), 0o644)
}
