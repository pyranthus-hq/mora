...
1. Seeds the page cursor from the stored `Checkpoint` (normally `""` after a clean prior run) and keeps the provider's between-run position separately in `IncrementalCursor`.
...
3. Durably publishes each advanced page `Checkpoint`, then **clears** it on clean completion. A supported incremental connector starts its next run from `IncrementalCursor`; a snapshot-only connector starts a new window from page one.
...
- **Snapshot fallback remains bounded.** Lean default windows keep initial snapshots and non-incremental connectors affordable. Gmail and Calendar normally consume provider deltas; an expired provider cursor triggers exactly one bounded snapshot fallback. A larger lookback is opt-in (`--since-days N`, persisted on the source, or one-shot `mora reingest --full`).
...
- **Skip-if-fresh.** The connect backfill skips a source whose `LastSuccessAt` is inside `connectFreshWindow` (1h) — a re-auth minutes after a clean backfill must not repeat provider work. `mora sync google` remains the explicit force path and never skips. Gmail `history.list` and Calendar `syncToken` positions live in `IncrementalCursor`; the legacy provider-specific status fields remain compatibility-only.
...
- **Clean finish ⇒ clear page checkpoint + stamp success + conditionally reset errors.** At the end of pagination the loop clears `Checkpoint`, stamps `LastSynced`/`LastAttemptAt`/`LastSuccessAt`, and resets the last error only when the run added none. The separate `IncrementalCursor` remains the next provider-native starting point.
- **Each completed page is durable before the next fetch.** The ingest callback saves an advanced `Checkpoint` after its writes and before requesting another page. A returned error or hard stop therefore resumes at the last unpublished page; clean completion saves the cleared checkpoint. Idempotent content-hash writes protect a repeated boundary page.
...
- **A clean run clears only the page checkpoint.** `Checkpoint` is intra-run resume state; `IncrementalCursor` is the committed between-run provider position. Never conflate them: publishing a provider cursor before all changed pages materialize could silently skip evidence after a crash.
- **`GmailHistory` / `CalSyncToken` are legacy compatibility fields.** New code reads and writes `IncrementalCursor`; the old names remain decodable so existing status files are not destructively rewritten.
- **Provider deletions are explicit.** Gmail history deletions and cancelled Calendar events arrive as tombstones and remove the corresponding local provider record. Filesystem manifests similarly detect vanished files. Snapshot-only connectors remain upsert-only: absence from a narrowed window is not evidence of deletion.
...
- **The sources.json registry serializes its read-modify-write behind a lease (P3).** Every `load → mutate → save` on the source registry goes through `internal/registry.SourceStore.Mutate`, which holds a crash-safe file lease at `<ConfigDir>/sources.json.lock` around the WHOLE cycle and **reloads the registry inside the lease**. `saveSources` + `atomicio.Write` already made each individual write collision-free (a unique temp per writer), but two processes — e.g. a manual `mora connect` racing the scheduled `ingest run --all` — each doing load→mutate→save could still lose an update: last `os.Rename` wins, silently dropping the other's enable bit / deny-list / persisted window. The lease closes that hole. **WHY reload-inside-lock:** loading before the lease reintroduces the race — the fix is that the second writer re-reads the first writer's *committed* state before it mutates, so no field is clobbered. The store uses `internal/leasefile`'s guarded publish/reap/compare-and-remove primitives, so it is Windows-portable. A lease leaked by SIGKILL is force-reaped after `registry.SourceLockTTL` (30s). The brief commit lock uses the sibling crash-released kernel-guard model because its critical section may span the whole render transaction. The sources lease is scoped to load→mutate→save ONLY — `connectFilesystem` releases it *before* its ingest/rebuild, so a long backfill never holds it. **Adding a new registry writer:** route it through the Mora `mutateSources` adapter / `registry.SourceStore.Mutate` (or acquire `SourceStore` directly if custom load-error handling is required) — a bare `loadSources`→`saveSources` outside the lease reopens the lost-update hole.
...
- **Crash-resume boundary:** every completed page checkpoint is persisted before the next fetch. A hard kill can repeat the currently unpublished page, but cannot force completed earlier pages to be fetched again. The idempotent content-hash upsert makes the boundary repeat safe. The `brief/.lock` remains a separate kernel-owned lock whose ownership is released automatically on process exit.
