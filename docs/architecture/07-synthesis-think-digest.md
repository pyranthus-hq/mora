...
Before urgency, recurrence collapse, ranking, or caps, the digest gates candidates on the generation-stamped whole-vault commitment snapshot: only open, canonical commitments whose opening evidence falls inside the trailing seven 24-hour periods ending at the injected clock (inclusive at both endpoints) can enter the DAILY surface. Filtering before caps prevents irrelevant artifacts from consuming a scarce slot. `attachDigestCommitments` then copies `Owner`, `Direction`, `DueAt`, `Lifecycle`, and `ClosureRef` from that same filtered snapshot. It never re-runs classification over a clipped snippet. For a multi-commitment artifact, the one artifact-grain lane deterministically selects the independently anchored opening visible latest in the title/snippet projection, falling back to stable evidence order when clipping hides every opening.

The same `DigestItem` then feeds both transports. CLI Markdown adds an indented `obligation:` line carrying the five typed values beneath the cited item. MCP returns the fields structurally in `urgent[]` or `sections[].items[]`. The current-surface exam adapters copy those values directly. They do not derive direction from section placement or recompute lifecycle from displayed text.
...
**CLI — `renderDigest` (`digest.go`)** emits Markdown: a `# Mora digest — <generated> (since last brief)` (or `(last Nh)` in window mode) header, a sorted `Fresh as of:` line, one `## <heading>` section per instance, and `## Open tasks (N stale)`. `sectionHeading` renders the three-state sentinel — `— no changes since last brief` / `— stale (no recent sync)` / `— unavailable (sync error)` / `— baseline (N)` / `(N)` for a live delta. `changePrefix` prepends `[new]`/`[updated]`. A typed item adds one indented `obligation:` line. And the `+N more since last brief` line surfaces the guard. The structural budgeter costs the complete rendered item, including its obligation line, so it is never clipped away from the cited parent item. `mora pulse --digest` layers `styleDigestTTY` *only on a TTY*. Pipes/redirects get the raw Markdown — the byte-clean invariant (see [cli-and-ux](./08-cli-and-ux.md)).
...
- The payload is `{generated, since_hours, urgent, source_states, freshness, stale_tasks, sections}` — no `digest` key. Each typed `DigestItem` in `urgent` or `sections[].items` carries the same obligation fields the CLI line renders.
