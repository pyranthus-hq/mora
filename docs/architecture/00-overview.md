...
| `internal/health` | `internal/operation` | Canonical source/index/producer/activity health DTOs and state vocabularies; deterministic connector freshness, projection-lag math, fail-closed worst-of aggregation, cross-arm priority, and bounded one-line source/index/share/producer/activity banner rendering. Mora retains config/database/journal/ledger loading, sync-status persistence, failure stamping, doctor checks, and surface orchestration. |
