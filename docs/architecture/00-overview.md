...
| `internal/governance` | `internal/atomicio`, `internal/commitment`, `internal/leasefile` | Vault-resident governance atom/entry DTOs, exact JSON ledger storage, fail-loud load, 30-second cross-process lease with bounded acquisition, serialized reload-under-lock append/revoke, ID/timestamp/author stamping, and pure identity normalization. Mora retains destructive CLI authority, suppression/Teach/merge projections, vault scans, Markdown publication, pending-op/index ordering, and connector orchestration. |
