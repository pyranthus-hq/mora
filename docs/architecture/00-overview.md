...
| `internal/mora` | all connector packages + `internal/atomicio`, `internal/genericutil`, `internal/storage` + lipgloss, go-isatty, go-selfupdate | The hub: CLI dispatch, connector wiring, filesystem ingestion, Markdown render/parse, SQLite index and search, derived entity graph, synthesis, MCP server, doctor, scheduler, eval harness, and self-update. |
