...
| `internal/sharing` | `internal/atomicio`, `internal/memory`, `filippo.io/age` | Transport-neutral sharing contracts, security, and encrypted bucket transport: durable ledgers/paths, strict grammar, canonical locators, signing/TOFU/freshness, manifest-pointer publish, ciphertext-only audits, bounded fail-closed fetch/materialization, content addressing, and orphan cleanup. Mora retains S3/git adapters, consent, publication/import governance, subscription mutation, and orchestration. |
