...
- **`index.db` carries a schema stamp (`PRAGMA user_version`) plus a physical v5 readiness check.** Bump `indexSchemaVersion` whenever the rebuilt shape changes meaning. Readers refuse a mismatch or an incomplete same-stamp D+E shape (actionable error), and `mora upgrade` re-stamps by rebuilding with the new binary.
