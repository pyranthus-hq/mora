...
- **New user memories are create-exclusive, not last-writer-wins** (`createMemory`/`atomicCreate`, functions `createMemory`/`atomicCreate`). `mora write` and MCP `write_memory` publish via `os.Link` (fails `EEXIST`) rather than `atomicWrite`'s replacing `os.Rename`; on a colliding `newID()` the loser re-mints and retries (bounded, `maxCreateAttempts`). WHY: two concurrent writers can mint the same second-granular id, and `os.Rename` would silently clobber one — real data loss. Updates and connector re-writes stay on `atomicWrite` (idempotent overwrite of a stable path is correct there). Mirrors `loop.go`'s `publishLockFile`; portable to Windows (`os.Link` = `CreateHardLinkW`, fails on a present target).
