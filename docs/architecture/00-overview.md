...
| `internal/synthesis` | `internal/memory`, `internal/search` | Deterministic think evidence and gap contracts, provenance/snippet construction, pre-model freshness/density/source/temporal gap facts, and outcome-state classification. Mora retains index/entity enrichment, retrieval caveats, open-loop/share joins, prompt orchestration, and CLI/MCP wiring. |
