...
| `internal/graph` | `internal/embed`, `internal/identity`, `internal/memory`, `internal/salience` | Pure deterministic compiler from memories plus explicit confirmed-merge facts to materialized entity, edge, candidate, merge-provenance, warning, gazetteer, and salience outputs. Mora owns governance loading, SQLite transaction/persistence, retrieval orchestration, CLI/MCP, and narrow compatibility adapters. |
...
| `internal/commitment` | standard library | Shared obligation direction/lifecycle and due-kind vocabulary, explicit/relative deadline classification, and stable length-framed evidence identity. Mora retains source-specific speech/evidence extraction, lifecycle inference, deduplication, materialization, and governance. |
...
| `internal/appupdate` | `github.com/Masterminds/semver/v3`, `github.com/creativeprojects/go-selfupdate`, `golang.org/x/sys/unix` (Darwin) | Fail-closed manual whole-app update transaction: canonical release/asset selection, bounded HTTPS downloads and redirects, checksum and ZIP preflight, signed/notarized bundle verification, same-volume atomic swap, rollback, and recovery preservation. Mora injects version decisions and post-swap index rebuilding; unattended policy, receipts, health fences, and CLI routing remain in the composition root. |
...
| `internal/search` | `internal/config`, `internal/embed`, `internal/genericutil`, `internal/memory`, `modernc.org/sqlite` | Query/filter parsing, canonical pre-rank FTS/vector reads, deterministic reciprocal-rank fusion and MMR reranking, strict whole-row lexical coverage, owner-aware returned-row recovery, previews, clustering, JSON budgets, and context assembly. Connector facts, index auto-heal/rebuild, retrieval-arm orchestration, visibility governance, later-related annotation, and CLI/MCP wiring remain in `internal/mora`. |
| `internal/sharing` | `internal/atomicio`, `internal/memory`, `filippo.io/age`, pure-Go AWS SDK | Sharing contracts, security, encrypted bucket transport, and S3-compatible adapter: durable ledgers/paths, strict grammar, environment-only credentials, canonical locators, signing/TOFU/freshness, manifest-pointer publish, ciphertext audits, bounded fail-closed fetch/materialization, content addressing, and orphan cleanup. Mora retains CLI consent, git transport, publication/import governance, subscription mutation, and orchestration. |
...
| `internal/urgency` | `internal/memory` | Deterministic urgent-item mechanics: Gmail actionability labels, score boosts, recency gate, deadline/negation matching, Unicode-safe deadline-centered snippets, and connector envelope stripping. Mora supplies human-sender/occurred-time facts and owns shelf assembly/budget policy. |
| `internal/update` | `internal/atomicio`, `github.com/Masterminds/semver/v3` | Strict durable update evidence store: state-directory receipt path, bounded single-object JSON, duplicate/unknown-field rejection, canonical versions, timestamp/order/skew validation, allowlisted outcomes, and atomic `0600` persistence. Mora injects the clock and retains policy, networking, notification, signing, swap, rollback, health, and CLI orchestration. |
...
| `internal/meeting` | `internal/memory` | Deterministic next-event selection: RFC3339 start derivation, timed/all-day windows, grace/horizon bounds, attendee-match gating, current/upcoming precedence, stable ties, and public event shape. Mora supplies identity membership and owns meeting-brief assembly. |
...
| `internal/mcp` | `internal/atomicio`, `internal/config`, `internal/memory` | MCP protocol and public contract: canonical initialization handshake/tool catalog, framing/results, method routing, strict schemas, budgets, mutation authority/proposals, and `write_memory` input construction. Mora loads config, binds the exact handler set, and retains handlers, governed vault mutation, decision policy, health/usage, and orchestration. |
...
| `internal/health` | standard library | Deterministic connector freshness thresholds and state classification, failed/never/stale/fresh precedence, worst-source selection, bounded single-line error sanitization, and health-banner rendering. Mora retains source loading, sync-status path/persistence wiring, failure stamping, and doctor/brief integration. |
...
| `internal/identity` | (leaf — no internal deps) | Pure deterministic person/service/org/repo/artifact classification, shortcode/phone recognition, precision-first sender heuristics, and structural-noise detection. Mora composes classifications into graph, salience, meeting, sharing, and public entity policy. |
