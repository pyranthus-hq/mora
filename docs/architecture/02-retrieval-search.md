...
One subtlety: when `vecOK`, the embedder is resolved **once** (`emb = chooseEmbedder()`, `hybrid.go:125`) and reused for both production and deep-trace vector arms, because a daemon dropping between two `chooseEmbedder()` calls would query an Ollama-keyed index with a static vector (dim/model mismatch → empty arm), silently corrupting the trace (`hybrid.go:121-124`).
...
- **The recall progression 0.591 → 0.667 → 0.727.** Two of these are code-anchored: FTS recall@5 **0.591 → 0.667** from dropping stopwords (`mora.go:3460-3461`), and the static-hash hybrid regression **0.591 → 0.394** (`hybrid.go:57`). The hybrid-static-with-stopword-fix figure **0.394 → 0.439** is also in `mora.go:3461`. The **0.727** figure (presumably FTS-or-hybrid recall@5 with Ollama on) does **not** appear in the code comments or in `docs/SESSION-2026-06-06-t2-eval.md`. The live-vault Ollama A/B verdict is stated only in `CLAUDE.md:100` as a histogram shift (**RETRIEVAL 2→0, HIT 10→13**, the `TestEvalAB` verdict), not a recall scalar — and that figure is **not** in the SESSION doc (whose lines 94–97 report a separate *synthetic* run: FTS-only `Recall@5=0.333` vs hybrid `Recall@5=1.000`, which the doc itself flags as a demonstration, not the verdict, `docs/SESSION-2026-06-06-t2-eval.md:100-105`). Treat 0.727 as unverified from this subsystem's code until a numeric Ollama-on recall@5 is located in the eval output.
- **Live Ollama recall win is asserted by docs, not reproducible from a static checkout.** The win requires `ollama pull nomic-embed-text` + `MORA_EMBEDDER=ollama` + re-index + `MORA_EVAL_LIVE=1` against a real vault (`CLAUDE.md`); it cannot be confirmed from the committed synthetic corpus, which the eval session notes is "too small / lexically leaky" to produce a true static-hash RETRIEVAL miss (`docs/SESSION-2026-06-06-t2-eval.md:102-105`).
