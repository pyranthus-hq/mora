...
func leaseGuardPath(lockPath string) string {
	resolved, identity := leaseGuardIdentity(lockPath)
...
func leaseGuardIdentity(lockPath string) (resolved, identity string) {
...
func withLeaseFileGuard(lockPath string, fn func() error) (err error) {
	...
	if err := lockLeaseGuard(f); err != nil {
		return err
	}
	defer func() { err = errors.Join(err, unlockLeaseGuard(f)) }()
	return fn()
}
