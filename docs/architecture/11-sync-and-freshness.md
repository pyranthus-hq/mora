...
- Filesystem sources: `filesystem-<source>.json`. The filesystem connector has no fetcher `Status` of its own, so `ingestFilesystem` (`internal/mora/ingest.go`) writes the `SyncStatus` itself after the walk — `LastSuccessAt`/`LastAttemptAt` on a clean walk, `ErrorCount`/`LastError` otherwise. Without this the digest's `classifyState` found no status (`syncStatusPathFor` returned `""`) and mislabelled an ingested filesystem source as **`unavailable (sync error)`**; the `filesystem` case in `syncStatusPathFor` + this write fix it so the Files section reads its real state.
