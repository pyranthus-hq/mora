...
| `internal/mcp` | `internal/atomicio`, `internal/config`, `internal/memory` | MCP protocol and public tool contract: canonical ordered catalog/strict schemas, framing/results, method routing, budgets, mutation authority/proposals, and `write_memory` input construction. Mora binds the catalog to an exact handler set and retains handlers, governed vault mutation, decision policy, health/usage, and orchestration. |
