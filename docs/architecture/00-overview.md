...
| `internal/mcp` | (leaf — no internal deps) | MCP protocol and budgeting mechanics: JSON-RPC framing, request caps, result envelopes, strict tool schemas/argument coercion, context-profile defaults/ceilings, token heuristics, overflow-safe clamp-before-conversion, and envelope payload budgets. Initialization, dispatch, governance, and content handlers remain in `internal/mora`. |
