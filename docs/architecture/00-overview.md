...
| `internal/ingest` | `internal/atomicio`, `internal/config`, `internal/memory` | Connector-ingest journal/lease lifecycle plus sync-status mechanics: canonical paths, remote/local freshness thresholds, honest attempt classification, error-preserving persistence, and best-effort freshness inventory. Connector fetch/dispatch and command presentation remain in `internal/mora`. |
