...
| `internal/mcp` | (leaf — no internal deps) | MCP protocol, budgets, and connection mutation authority: framing/results, schemas/arguments, context ceilings, canonical initialization instructions, write-policy normalization, and execute/propose/refuse decisions (including no destructive proposals). Proposal storage, tool registry/handlers, health/usage, and orchestration remain in `internal/mora`. |
