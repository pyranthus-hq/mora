...
| `internal/index` | `internal/config` | Embedded SQLite index storage boundary: canonical database path, fail-closed read-only URI, serialized WAL-writer DSN, schema-version fencing, and incremental-schema readiness probes. Stage B extraction; rebuild, auto-heal policy, graph/vector construction, and CLI wiring remain in `internal/mora`. |
