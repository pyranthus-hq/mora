...
| `internal/config` | (leaf — no internal deps) | Home of the shared `Config` contract, including durable paths/settings and accessor-guarded process-local ingest/retrieval/vault overrides. Stage B extraction; `internal/mora` aliases the type while configuration I/O remains at the composition boundary pending the next bottom-up move. |
