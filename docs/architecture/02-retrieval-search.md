...
with RRF. A router selects FTS-only or hybrid search from measured recall.
...
flowchart TD
    ...
    R -->|"static-hash<br/>(default, or Ollama daemon down)"| F["searchMemories<br/>parent + Gmail segment FTS<br/>RRF only when segment arm participates"]
