...
| `internal/ingest` | `internal/atomicio`, `internal/config`, `internal/google`, `internal/memory` | Connector-ingest mechanics: journal/lease lifecycle, sync-status honesty/freshness, operation/source identity, Google token paths, connector fetch-window policy, and filesystem curation/name classification. Fetch/dispatch, governance writes, and commands remain in `internal/mora`. |
