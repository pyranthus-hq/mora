...
Codex can call ten tools for memory, search, the entity graph, and synthesis.
...
| `read_memory` | Fetch one memory by id (full body, a bounded match-centred excerpt, or one Gmail evidence segment) | `id`*, `match`, `max_tokens`, `occurrence`, `evidence_ref` | parameter-free payload is `{memory, health}` (full body, byte-identical to before #242). Any of `match`/`max_tokens`/`occurrence` opts into bounded mode: `memory.text` becomes the excerpt and a sibling `{receipt}` key is added. `evidence_ref` (issue #243) narrows the target to ONE derived Gmail message segment first, then `match`/`max_tokens`/`occurrence` apply within that narrowed text; a `receipt` is always added, and a ref that does not belong to `id` is an explicit error |
| `search_memory` | Most-relevant memories. Hybrid only when a semantic embedder is active, else FTS-only | `query`*, `scope`, `limit`, `source`, `since_hours` | `limit` = `mcpSearchDefaultLimit` = **8**. Bodies snippeted to 240 runes. Payload is `{results, freshness, health}`, plus `filters`/`excluded_by_filter` when a source/since_hours filter is applied (#241 below). A Gmail thread result with a matching derived message segment carries `evidence: {evidence_ref, sender, at, snippet}` naming its strongest matching message (#243) |
| `list_memory` | Recent memory previews, newest first | `scope`, `limit` | `limit` = **10**. Bodies are head-clipped to `searchSnippetLen` (**240** runes), `Meta` is omitted, and aggregate row drops are reported as `memories_truncated`. Use `read_memory` for the full record. Payload is `{memories, health}` |
