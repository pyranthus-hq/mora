...
| `internal/commitment` | `internal/evidence`, `internal/evidencetext`, `internal/identity` | Typed derived commitment record/span/citation JSON model plus provider-neutral speech ownership/manual-promise adjudication plus shared obligation direction/state/due vocabulary, deadline classification, stable evidence identity, deterministic guarded lifecycle transitions with typed closure citations, ambiguity gaps, citation union, stable-ID uniqueness, and provenance-gated duplicate/canonical projections. Mora retains provider-specific actor/party/evidence extraction, provider participant/name extraction and citation construction, and lifecycle-evidence adaptation from connector-admitted messages, health-aware materialization, SQLite generation ownership, and governance application. |
