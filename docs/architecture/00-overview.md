...
| `internal/mora/mora.go` | 5605 | The hub: CLI dispatch (`Run`), wiring boundary (`writeMappedMemory`), index pipeline (`rebuildIndex`), MCP server (`serveMCP`). Cohesive subsystems are progressively split into same-package sibling files (e.g. `doctor.go`, `schedule.go`, `vaultops.go`, `usage.go`). |
