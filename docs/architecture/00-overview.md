...
4. **Retrieve** — Search has two paths. `defaultSearch` (`hybrid.go`) routes to the **static keyword surface (parent FTS + bounded Gmail message-segment FTS)** under the static-hash embedder and to **hybrid (RRF-fused parent FTS + vector + 1-hop graph + bounded Gmail message-segment FTS) only when a semantic embedder is genuinely active**. The semantic gate preserves the measured static baseline because the ungated graph arm would still perturb its ranking. See [retrieval & search](./02-retrieval-search.md) and [entity graph](./03-entity-graph.md).
...
| [02 — Retrieval & Search](./02-retrieval-search.md) | Two search paths, the embedder gate (`defaultSearch`), bounded Gmail message-segment retrieval, four-arm RRF fusion, the static-hash vs Ollama embedders, FTS stopword handling, and zero-egress. |
...
- **embedder-gated routing** — Search routes via `defaultSearch`, which inspects the *actually-active* embedder (`chooseEmbedder`): the static keyword surface (parent FTS + bounded Gmail message-segment FTS) under static-hash, hybrid only when semantic. Because a vector-omitting hybrid still perturbs RRF via its graph arm, the gate is on the resolved embedder, not raw `MORA_EMBEDDER`.
...
- **RRF (Reciprocal Rank Fusion)** — How hybrid fuses its four arms (`rrfWeighted`, `hybrid.go`, production k=10): parent FTS, vector, graph, and Gmail message-segment rankings contribute by rank, so unbounded BM25 and [0,1] cosine scores need no normalization.
