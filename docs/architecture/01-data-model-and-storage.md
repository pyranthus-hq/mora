...
- Vectors feed only the **hybrid** retrieval path, which `defaultSearch` enables **only** when a semantic embedder (Ollama) is configured. Under the default static-hash embedder, `defaultSearch` uses the static keyword surface (parent FTS plus bounded Gmail message-segment FTS), so a missing vector has no effect there. Under a semantic embedder the new memory is a **real but bounded, self-healing recall gap** on the vector arm — fully searchable via FTS immediately, and it gains its vector at the next full rebuild.
