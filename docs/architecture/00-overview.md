...
| `internal/hooks` | `internal/atomicio` | Fail-closed Claude settings management: unknown-field-preserving JSON, safe path/load semantics, atomic 0600 writes, idempotent managed-hook install/update, selective uninstall, and status detection. Mora retains CLI flags/output plus session-start and recall orchestration. |
