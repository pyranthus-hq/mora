	...
	for len(remaining) > 0 {
		...
		for j, c := range remaining { // remaining stays in fused order; strict > ⇒ lowest fused rank wins ties
			...
			for _, s := range selected {
				if sim := clampPos(embedpkg.Cosine(vecByID[c], vecByID[s])); sim > maxSim {
					maxSim = sim
				}
			}
...
func loadVectorsByID(ctx context.Context, db *sql.DB, model string, ids []string) (map[string][]float32, error) {
	...
	for rows.Next() {
		...
		out[id] = embedpkg.DecodeVec(b)
	}
	return out, rows.Err()
}

func MMRRerank(ids []string, rel map[string]float64, vectors map[string][]float32, lambda float64) []string {
	return mmrRerank(ids, rel, vectors, mmrParams{lambda: lambda})
}
func MMRActive(useVector, force bool) bool { return mmrActive(useVector, &mmrParams{force: force}) }
func LoadVectorsByID(ctx context.Context, db *sql.DB, model string, ids []string) (map[string][]float32, error) {
	return loadVectorsByID(ctx, db, model, ids)
}

const DefaultMMRLambda = defaultLambda

func ClampPositive(x float64) float64 { return clampPos(x) }
