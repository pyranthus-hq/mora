...
// Manifest is the full current set for one publisher/scope. It carries no
...
// Envelope wraps the manifest for transport. Version is mirrored OUTSIDE
...
type Envelope struct {
	...
	Sig       []byte `json:"sig"` // ed25519 over SigningMessage(Payload, Version)
}

// BlobKeyRE gates object keys on a bucket/relay to content-addressed
// ciphertext only — the namespace equivalent of git's ls-files allowlist.
var BlobKeyRE = regexp.MustCompile(`^[0-9a-f]{64}\.age$`)
...
func SigningKeyPath(configDir string) string {
	return filepath.Join(configDir, "share", "signing.key")
}

// SigningKey loads (or, on first use, creates) the publisher's ed25519
...
func SigningKey(configDir string) (ed25519.PrivateKey, error) {
	path := SigningKeyPath(configDir)
...
// SigningMessage binds the LOCATOR and the version — both of which live
...
func SigningMessage(locator string, payload []byte, version int) []byte {
...
func SealManifest(priv ed25519.PrivateKey, locator string, manifestJSON []byte, version int, recipients []age.Recipient, encrypt bool) (Envelope, error) {
	payload := manifestJSON
	if encrypt {
		ct, err := EncryptBytes(recipients, manifestJSON)
		if err != nil {
			return Envelope{}, err
		}
		payload = ct
	...
	if !ok {
		return Envelope{}, errors.New("share manifest: signing key has no ed25519 public half")
	}
	return Envelope{
		Payload:   payload,
		Encrypted: encrypt,
		Version:   version,
		SignPub:   pub,
		Sig:       ed25519.Sign(priv, SigningMessage(locator, payload, version)),
	}, nil
}
...
func VerifyEnvelope(env Envelope, locator string, pinnedPub ed25519.PublicKey, lastVersion int) error {
	...
	if len(env.Sig) != ed25519.SignatureSize ||
		!ed25519.Verify(ed25519.PublicKey(env.SignPub), SigningMessage(locator, env.Payload, env.Version), env.Sig) {
...
// MaxManifestBytes caps the manifest — the index of the WHOLE set, so much
...
func OpenManifestPayload(env Envelope, identities []age.Identity) ([]byte, error) {
	if !env.Encrypted {
		if len(env.Payload) > MaxManifestBytes {
			return nil, fmt.Errorf("share manifest is %d bytes, over the %d-byte cap — refusing", len(env.Payload), MaxManifestBytes)
		}
	...
	plain, err := io.ReadAll(io.LimitReader(r, MaxManifestBytes+1))
	if err != nil {
		return nil, err
	}
	if len(plain) > MaxManifestBytes {
		return nil, fmt.Errorf("decrypted share manifest exceeds the %d-byte cap — refusing", MaxManifestBytes)
	}
	return plain, nil
}
...
func EncryptBytes(recipients []age.Recipient, plaintext []byte) ([]byte, error) {
