...
- **`scanner.Scan()` uses the default 64KB token limit.** A single inbound request line longer than `bufio.MaxScanTokenSize` would terminate the loop. Inbound tool-call args are small, so this has not bitten, but a future large `write_memory` text passed inline could.
...
- **Usage logging is local-only and opt-out-honoring.** Stays in `<StateDir>/usage/`, never the vault, never the network; `DO_NOT_TRACK=1` or the `OFF` sentinel disables it. WHY: zero-egress is a product promise (see [distribution-and-ops](./10-distribution-and-ops.md)).
...
- **64KB line limit.** `serveMCP` uses a default `bufio.Scanner` with no `scanner.Buffer` override. A `write_memory` whose inline `text` (plus envelope) exceeds 64KB on a single request line would terminate the read loop. Not exercised by any test I found; treat as a latent edge.
