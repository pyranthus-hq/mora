...
The frozen corpus directories contain adjudication evidence only. Product-output ratchets such as `report.golden.tsv`, real predictions, flywheel predictions, and surface scorecards live under `exam/testdata/`, so a product fix can advance measured output without rewriting sealed `eval/obligations-v1|v2` bytes.
