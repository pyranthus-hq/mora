...
type Ranking struct {
	All           []Result
	Selected      []Result
	ThinAttendees []string
}

func (r Ranking) ByID(id string) Result {
...
func defaultForgettabilityOptions(in Options, policy Policy) Options {
	out := in
	...
	if out.ThinCoverageK <= 0 {
		out.ThinCoverageK = policy.ThinCoverageK
	}
	if out.PerAttendeeCap <= 0 {
		out.PerAttendeeCap = 3
	}
	if out.EvidenceCap <= 0 {
		out.EvidenceCap = policy.EvidenceCap
	}
	return out
}

func Rank(now time.Time, eventTitle string, attendeeNames []string, candidates []Candidate, opts Options, policy Policy) Ranking {
	opts = defaultForgettabilityOptions(opts, policy)
	eventTokens := forgettabilityDistinctiveTokens(eventTitle, attendeeNames, policy)
	tokenSets := make(map[string]map[string]bool, len(candidates))
	for _, c := range candidates {
		tokenSets[c.StableID] = forgettabilityDistinctiveTokens(c.Title+" "+c.Text, attendeeNames, policy)
	}
	...
	for _, c := range candidates {
		...
		if c.MentionCount > 0 && c.MentionCount < opts.ThinCoverageK && !thinSeen[c.PersonID] {
			...
			ranking.ThinAttendees = append(ranking.ThinAttendees, "Only 1 memory about "+name+" - coverage is thin.")
...
func scoreForgettabilityCandidate(now time.Time, eventTokens map[string]bool, tokenSets map[string]map[string]bool, all []Candidate, c Candidate, opts Options) Result {
...
func sortForgettabilityResults(results []Result) {
...
func maxNewerOverlap(c Candidate, all []Candidate, tokenSets map[string]map[string]bool) float64 {
	tFact, dated := parseForgettabilityTime(c.OccurredAt, c.CreatedAt)
	if !dated {
		return 0
	}
...
func forgettabilityDistinctiveTokens(s string, attendeeNames []string, policy Policy) map[string]bool {
	exclude := map[string]bool{}
	for _, name := range attendeeNames {
		for _, tok := range policy.Tokenize(name) {
			exclude[tok] = true
		}
	}
	out := map[string]bool{}
	for _, tok := range policy.Tokenize(s) {
		if len(tok) < 3 || policy.IsStopword(tok) || exclude[tok] {
			continue
		}
		out[tok] = true
	}
...
func (g Gates) all() bool {
	return g.Human && g.IdentityKnown && g.NotSelf && g.NotBulk && g.NotDeleted && g.Valid
}

func DistinctiveTokens(s string, names []string, policy Policy) map[string]bool {
	return forgettabilityDistinctiveTokens(s, names, policy)
}
func IntersectionSize(a, b map[string]bool) int { return intersectionSize(a, b) }
func ParseTime(occurredAt, createdAt string) (time.Time, bool) {
	return parseForgettabilityTime(occurredAt, createdAt)
}
