...
This fixed, model-free pipeline builds a person and topic graph. It uses
connector metadata and message bodies. It marks each identity as `person` or
`service`. It joins one person's addresses into one standard node. Strict
matching comes first, and each rebuild gives the same bytes.
...
Connector metadata is the only input: `Meta["from"]`, `["to"]`, `["cc"]`,
`["attendees"]`, `["organizer"]`, `["participants"]`, and `["names"]`.
Connectors fill these fields at `internal/google/calendar.go:82` and
`internal/google/identity.go:129`. `mora` reads them at the wiring boundary.
See [connectors-google](./04-connectors-google.md) and
[connectors-imessage](./05-connectors-imessage.md).
