...
| `internal/search` | `internal/config`, `internal/genericutil`, `internal/memory`, `modernc.org/sqlite` | Query/CLI parsing, filters, canonical pre-rank FTS reads, previews, corroboration clustering, JSON result budgets, and control-file/memory context assembly. Stage B extraction; index auto-heal/rebuild, catalog validation, segment/hybrid fusion, visibility governance, later-related annotation, and CLI/MCP wiring remain in `internal/mora`. |
