...
| `internal/digest` | Presentation boundary: deterministic Markdown framing, urgent/section/obligation rendering, source-state headings, structural byte budgeting, fair per-source floors, and exact survivor-ID reporting. It receives plain render DTOs and never loads vault/state, computes health, or advances watermarks. |
