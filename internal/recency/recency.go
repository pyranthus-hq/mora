...
func indexedAtOf(m memory.Memory) (string, bool) {
	s := m.LastSynced
	if s == "" && m.Provider == "" {
		s = m.CreatedAt
	}
...
// sort ignored.
func ingestRecencyOf(m memory.Memory) (time.Time, bool) {
	s, ok := indexedAtOf(m)
	if !ok {
		return time.Time{}, false
	}
...
func byIngestRecency(a, b memory.Memory) bool {
...
func eventStartOf(m memory.Memory) (string, bool) {
...
func sourceCreatedAtOf(m memory.Memory) (string, bool) {
	if m.Meta == nil {
		return "", false
	}
	...
	if m.Provider == "gmail" {
		...
		if json.Unmarshal(b, &messages) == nil && len(messages) > 0 {
			if _, ok := rfc3339Instant(messages[0].At); ok {
				return messages[0].At, true
			}
		}
	}
	return "", false
...
func decorateBrowseRecency(mems []memory.Memory) []memory.Memory {
	if mems == nil {
		return nil
	}
...
func Instant(s string) (time.Time, bool)             { return rfc3339Instant(s) }
func IndexedAt(m memory.Memory) (string, bool)       { return indexedAtOf(m) }
func IngestTime(m memory.Memory) (time.Time, bool)   { return ingestRecencyOf(m) }
func Before(a, b memory.Memory) bool                 { return byIngestRecency(a, b) }
func EventStart(m memory.Memory) (string, bool)      { return eventStartOf(m) }
func SourceCreatedAt(m memory.Memory) (string, bool) { return sourceCreatedAtOf(m) }
func Decorate(mems []memory.Memory) []memory.Memory  { return decorateBrowseRecency(mems) }
