...
| `internal/mora/mora.go` | 4775 | The hub: CLI dispatch (`Run`), wiring boundary (`writeMappedMemory`), index pipeline (`rebuildIndex`), MCP server (`serveMCP`). Cohesive subsystems are progressively split into same-package sibling files (e.g. `doctor.go`, `schedule.go`, `usage.go`, `tasks.go`, `search.go`, `memfile.go`). |
