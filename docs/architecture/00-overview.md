...
| `internal/mcp` | `internal/atomicio`, `internal/config`, `internal/memory` | MCP protocol, method routing, budgets, mutation authority/proposals, and `write_memory` input construction: framing/results, deterministic initialize/list/call dispatch, schemas/arguments, context ceilings, policy-consistent authority, proposal integrity, and canonical pre-publish records. Tool registration/handlers, governed vault mutation, decision policy, health/usage, and orchestration remain in `internal/mora`. |
