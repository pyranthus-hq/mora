...
This is the single fact that keeps the visible-brief layer honest. `writeBriefArtifact` **never calls `briefstate.Save` / `briefstate.AcquireLock`** — it is pure render + `atomicio.WriteDurable`, local-only (no `net/*` import). So persisting a brief is *not* "I read it": the Phase-12 delta watermark stays gated **solely on `--advance`** (D-02/SC#4). Two distinct stores, two distinct directories, deliberately not conflated:
