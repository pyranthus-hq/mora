...
| `internal/leasefile` | `internal/storage`, `golang.org/x/sys`, `golang.org/x/text` | Cross-process path-keyed guard placement, physical filesystem identity across aliases/recreation, and blocking/try/unlock primitives on Unix and Windows. Mora retains narrow compatibility adapters while lease consumers migrate. |
