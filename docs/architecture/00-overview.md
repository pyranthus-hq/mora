...
| `internal/mora/mora.go` | 2030 | The hub: CLI dispatch (`Run`), wiring boundary (`writeMappedMemory`, `ingest.go`), index pipeline (`rebuildIndex`, `index.go`), MCP server (`serveMCP`). Cohesive subsystems are progressively split into same-package sibling files (e.g. `doctor.go`, `schedule.go`, `usage.go`, `tasks.go`, `search.go`, `memfile.go`, `config.go`, `index.go`, `sources.go`, `setup.go`, `ingest.go`). |
