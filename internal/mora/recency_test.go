...
func recencyIMessageChat() Memory {
	return Memory{
...
func recencyGoogleEventWithCreation() Memory {
	return Memory{
...
func TestStrictRFC3339IsTheGrammarNotTimeParse(t *testing.T) {
	...
	t.Run("valid forms are accepted and preserved byte-for-byte", func(t *testing.T) {
		for _, s := range []string{
			...
			m := Memory{ID: "m", Type: "event", Meta: map[string]any{"occurred_at": s}}
...
func TestIndexedAtIsNeverFabricated(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			mem:  Memory{ID: "mem_bad_local", Source: "mcp", CreatedAt: "2026-07-30 12:00:00"},
		},
	}
...
func TestBrowseRowsSplitConflatedTimestamps(t *testing.T) {
	...
	for _, tc := range cases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			rows := decorateBrowseRecency([]Memory{tc.mem})
			if len(rows) != 1 {
...
func TestDerivedTimestampsOmitMalformedStamps(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			redone := decorateBrowseRecency([]Memory{poisoned})[0]
...
func TestSabotagedStampsAreOmittedFromEveryDerivedField(t *testing.T) {
	sites := []struct {
		...
		inject                            func(stamp string) Memory
	...
	realClock := Memory{ID: "gmail_ok", Provider: "gmail", CreatedAt: "2026-01-01T00:00:00Z", LastSynced: "2026-07-31T01:00:00Z"}
	...
	for _, site := range sites {
		for _, bad := range rfc3339Sabotage {
			...
			t.Run(fmt.Sprintf("%s/%s", site.field, bad.why), func(t *testing.T) {
				...
				got := decorateBrowseRecency([]Memory{mem})[0]
...
func TestBrowseRecencyComparesInstantsNotStrings(t *testing.T) {
	...
	local := Memory{ID: "mem_local_offset", CreatedAt: "2026-07-30T23:00:00-04:00"}
	connector := Memory{ID: "gmail_earlier", Provider: "gmail", CreatedAt: "2026-01-01T00:00:00Z", LastSynced: "2026-07-31T01:00:00Z"}
	...
	corrupt := Memory{ID: "mem_corrupt", CreatedAt: "not-a-timestamp"}
