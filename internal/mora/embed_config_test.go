...
func TestChooseEmbedderForPrecedence(t *testing.T) {
	...
	if got := ident(chooseEmbedderFor(Config{Embedder: "ollama"})); got != static {
		t.Fatalf("env=%q must override the config opt-in → static; got %q", "", got)
	}

	// 2. no env + no config ⇒ the static floor.
	os.Unsetenv("MORA_EMBEDDER")
	if got := ident(chooseEmbedderFor(Config{})); got != static {
	...
	os.Unsetenv("MORA_EMBEDDER")
	viaCfg := ident(chooseEmbedderFor(Config{Embedder: "ollama"}))
	os.Setenv("MORA_EMBEDDER", "ollama")
	viaEnv := ident(chooseEmbedderFor(Config{}))
	if viaCfg != viaEnv {
