...
- `edges.valid_to` is declared in the schema (`internal/mora/mora.go:2040`) but `writeGraph` always inserts it `NULL` (`internal/mora/mora.go:2131`) and no read path I own consults it — it appears reserved for a future bi-temporal close, unused in v1.
