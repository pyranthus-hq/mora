...
| `internal/meeting` | `internal/memory` | Deterministic next-event selection plus the pure meeting-evidence text boundary: quoted/forwarded/signature exclusion, URL/noise stripping, hard-wrap repair, sentence segmentation, speaker/subject/header guards, phrase boundaries, personal-trivia detection, and dated historical framing. Mora supplies identity/config/governance, classifies obligations, loads/ranks evidence, budgets the assembled brief, and owns CLI/MCP orchestration. |
