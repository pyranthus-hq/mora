...
func digestSynthesisPrompt(urgent []DigestItem, sections []DigestSection, states []sourceState) string {
	lowerUrgent := make([]digestpkg.Item, len(urgent))
	for i, item := range urgent {
		lowerUrgent[i] = digestpkg.Item{ID: item.ID, Source: item.Source, Title: item.Title, Snippet: item.Snippet}
	}
	lowerSections := make([]digestpkg.Section, len(sections))
	for i, section := range sections {
		items := make([]digestpkg.Item, len(section.Items))
		for j, item := range section.Items {
			items[j] = digestpkg.Item{ID: item.ID, Source: item.Source, Title: item.Title, Snippet: item.Snippet}
		}
		lowerSections[i] = digestpkg.Section{Source: section.Source, State: section.State, Items: items}
	}
	lowerStates := make([]digestpkg.SourceState, len(states))
	for i, state := range states {
		lowerStates[i] = digestpkg.SourceState{Instance: state.Instance, State: state.State}
	}
	return digestpkg.SynthesisPrompt(lowerUrgent, lowerSections, lowerStates)
}
