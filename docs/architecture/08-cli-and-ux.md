...
| `lint` / `backup` | Verify control files exist / tar.gz the vault to state dir. | Mora `cmdLint`/`cmdBackup` authorize and wire; `internal/vaultarchive.Write` owns fail-closed archive mechanics. |
...
| `upgrade [--check\|--policy auto\|notify\|off\|--status [--json]]` | Manual verified self-update plus leased internal scheduled policy: notify-only, zero-egress off, or health-gated verified whole-app auto apply with rollback evidence. No schedule is installed yet; status is network-free. | `cmdUpgrade` `upgrade.go`, `update_policy.go`, `internal/update/unattended.go` |
...
Note the parallel-but-distinct check `genericutil.IsInteractive(r io.Reader)` (`internal/genericutil/genericutil.go`), used for **stdin** (consent menus). It uses the stdlib `os.ModeCharDevice` test instead of go-isatty — acceptable there because the failure mode is "menu blocks on a non-TTY," not "escape codes leak into a machine stream." Do not conflate the two: writer-side TTY detection (output cleanliness) must stay on go-isatty. Reader-side (don't-block) can stay on the stdlib stat.
...
- **Operation activity** (`operation_activity.go`) — a read-only, sanitized `activities` array in doctor JSON and subordinate human rows for running/stalled/failed/completed ingest or rebuild work. Activity never makes a dirty index healthy; corrupt/future receipts fail closed.
