...
| `internal/mcp` | `internal/atomicio`, `internal/config`, `internal/memory` | MCP protocol, budgets, mutation authority/proposals, and `write_memory` input construction: framing/results, schemas/arguments, context ceilings, policy-consistent authority, proposal integrity, required fields/defaults, and decision-only validity routing. Governed vault mutation, decision normalization policy, handlers, health/usage, and orchestration remain in `internal/mora`. |
