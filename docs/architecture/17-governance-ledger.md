...
| File | Responsibility |
|---|---|
| `internal/mora/governance.go` | The whole primitive: the ledger types, load/save/append/revoke, the stable-atom key derivation from `Meta`, the suppression decision, and the guard entry point `shouldSuppressWrite`. |
...
| `internal/mora/ingest.go` | One added guard at the top of `writeMappedMemory` (the single connector write chokepoint). |
...
- **The guard is the only place re-ingest is blocked, and it cannot be bypassed by a new sync.** It lives inside `writeMappedMemory`, so all connector call sites (gmail/imessage/applecal) are covered without per-site edits. *Why:* a suppression the sync path doesn't read is silently reverted every hour.
- **The derived-attachment path needs its own guard.** `writeAttachmentMemories` (iMessage PDFs) mints `att_<hash>` ids that do **not** carry the parent's participant `Meta`, so the per-item guard can't see they belong to a forgotten chat. It checks the *parent's* suppression before extracting, so a forgotten conversation's PDFs are not smuggled in through this fifth write path. *Why:* the "single chokepoint" is really two — the primary and its derived sibling.
...
- **Filesystem sources are out of the guard's path.** `ingestFilesystem` bypasses `writeMappedMemory` (it renders directly, like its content-hash skip), so the ledger does not gate filesystem re-ingest. This is intentional for now: filesystem memories carry no participant identity, so `forget --handle/--email` never targets them, and forgetting a file source is better done by narrowing/disabling the source. Extending the guard to that path is a follow-up.
