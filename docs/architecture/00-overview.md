...
| `internal/search` | (leaf — no internal deps) | Safe FTS5 MATCH compilation, CLI search-argument parsing, and the shared trusted-source/time-window filter contract (receipts, canonical selectors, SQL predicates, fail-closed timestamp conversion). Stage B extraction; catalog-aware validation, retrieval execution, fusion, snippets, and context assembly remain in `internal/mora`. |
