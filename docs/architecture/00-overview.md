...
| `internal/commitment` | standard library | Shared obligation direction/state/due vocabulary, deadline classification, stable evidence identity, deterministic guarded lifecycle transitions, ambiguity gaps, and provenance-gated duplicate/canonical projections. Mora retains provider-specific speech/evidence extraction, citation construction/merging, health-aware materialization, SQLite generation ownership, and governance application. |
...
| `internal/segments` | `internal/memory`, `internal/urgency`, SQLite | Fail-closed Gmail/iMessage message-grain evidence derivation, exact projection schema, caller-transaction-bound prepare/write/clear operations, and low-level `(memory_id, evidence_ref)` lookup. Mora retains rebuild/upsert transaction ownership, index opening, provider orchestration, search/read shaping, and MCP policy. |
