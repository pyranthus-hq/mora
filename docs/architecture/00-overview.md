...
| `internal/sharing` | `internal/atomicio`, `internal/gitsync`, `internal/memory`, `filippo.io/age`, pure-Go AWS SDK | Sharing contracts, security, encrypted bucket transport, and S3-compatible adapter: durable ledgers/paths, strict grammar, environment-only credentials, canonical locators, signing/TOFU/freshness, manifest-pointer publish, ciphertext audits, bounded fail-closed fetch/materialization, content addressing, bounded generation/fetch/commit/private-Git-ref garbage collection with floor protection and one-attempt Windows deferral, and the fail-closed owner-fenced `attempt.json` store/recovery lifecycle, immutable generation layout, fail-closed highest-commit resolution, and corpus/index digest verification primitives. Mora retains CLI consent, git transport, live-lease interpretation, GC target/lease orchestration, storage admission, publication/import governance, fetch/build/commit ordering, subscription mutation, and orchestration. |
