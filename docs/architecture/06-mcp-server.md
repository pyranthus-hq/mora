...
| `read_memory` | Fetch one memory by id (full body, a bounded match-centred excerpt, or one Gmail evidence segment) | `id`*, `match`, `max_tokens`, `occurrence`, `evidence_ref` | parameter-free payload is `{memory, health}` (full body, byte-identical to before #242). Any of `match`/`max_tokens`/`occurrence` opts into bounded mode: `memory.text` becomes the excerpt and a sibling `{receipt}` key is added. `evidence_ref` (issue #243) narrows the target to ONE derived Gmail message segment first, then `match`/`max_tokens`/`occurrence` apply within that narrowed text; a `receipt` is always added, and a ref that does not belong to `id` (wrong memory, unknown, or a memory whose segments failed closed) is an explicit error |
| `search_memory` | Most-relevant memories. Hybrid only when a semantic embedder is active, else FTS-only | `query`*, `scope`, `limit` | `limit` = `mcpSearchDefaultLimit` = **8**. Bodies snippeted to 240 runes. Payload is `{results, freshness, health}`. A Gmail thread result with a query-matching derived message segment (issue #243) carries an additional `evidence: {evidence_ref, sender, at, snippet}` key naming its strongest matching message |
