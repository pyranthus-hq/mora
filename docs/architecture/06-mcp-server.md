...
| `read_memory` | Fetch one memory by id (full body, or a bounded match-centred excerpt) | `id`*, `match`, `max_tokens`, `occurrence` | parameter-free payload is `{memory, health}` (full body, byte-identical to before #242). Any of `match`/`max_tokens`/`occurrence` opts into bounded mode: `memory.text` becomes the excerpt and a sibling `{receipt}` key is added |
| `search_memory` | Most-relevant memories. Hybrid only when a semantic embedder is active, else FTS-only | `query`*, `scope`, `limit`, `source`, `since_hours` | `limit` = `mcpSearchDefaultLimit` = **8**. Bodies snippeted to 240 runes. Payload is `{results, freshness, health}`, plus `filters`/`excluded_by_filter` when a source/since_hours filter is applied (#241 below) |
