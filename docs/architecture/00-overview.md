...
| `internal/briefartifact` | `internal/atomicio` | Deterministic `<VaultDir>/briefs/<UTC-date>-brief.md` paths and crash-durable 0644 replace-per-day publication of caller-rendered bytes. Mora owns digest rendering, budgets, watermark authority, CLI/scheduler decisions, and supplies the injected timestamp. |
...
| `internal/sharing` | `internal/atomicio`, `internal/memory`, `filippo.io/age`, pure-Go AWS SDK | Sharing contracts, security, encrypted bucket transport, and S3-compatible adapter: durable ledgers/paths, strict grammar, environment-only credentials, canonical locators, signing/TOFU/freshness, manifest-pointer publish, ciphertext audits, bounded fail-closed fetch/materialization, content addressing, orphan cleanup, and the fail-closed owner-fenced `attempt.json` store/recovery lifecycle, immutable generation layout, fail-closed highest-commit resolution, and corpus/index digest verification primitives. Mora retains CLI consent, git transport, publication/import governance, fetch/build/commit ordering, subscription mutation, and orchestration. |
...
| `internal/meeting` | `internal/graph`, `internal/memory` | Deterministic next-event selection, sender-authored evidence text/historical framing, participant-roster gating, and pure meeting-evidence classification: notification/third-party/service exclusion, provider-specific ownership, actionable-question boundaries, unresolved/staleness/material-context policy. Mora supplies identity/config/service/time facts and retains attribution, commitments, governance, vault/graph loading, ranking, budgeting, and CLI/MCP assembly. |
