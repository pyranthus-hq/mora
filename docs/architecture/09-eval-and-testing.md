...
Three test tools measure what the code cannot prove about itself. The T2 eval
measures **search recall quality** and assigns each miss to a cause. The T0 gate
checks **MCP output size** and limits token growth for each tool. The
**meeting-brief sabotage gate** replays the cleaned July 2026 gibberish case
through the full production path.

The rendered-output obligation exam adds one typed ledger. It always renders
the same synthetic Gmail, iMessage, Calendar, and notes corpus. Its rebuild,
hash, and real-identity ban use AMD GAIA issue #848 as design evidence. They
also use the old generator at commit `0c197ef3` (MIT). Mora imports no GAIA code
or corpus.

A high, clean, uniform first score shows a ledger defect, not a win. Known
misses keep their labels. Thus, the exam measures the product and does not fit
the fixture to current output.
...
The frozen corpus directories contain adjudication evidence only. Product-output ratchets such as `report.golden.tsv`, real predictions, flywheel predictions, and surface scorecards live under `exam/testdata/`, so a product fix can advance measured output without rewriting sealed `eval/obligations-v1|v2|v3` bytes.
...
`TestExamIntegrityExit` is the Gate 1 trust certificate. For all three schema versions it requires validator and all five lint families to pass, rendered and checked-out bytes to match `CORPUS.sha256`, every scorecard metric to retain a registered sabotage case, and the determinism guard to cover the adapters. It also requires the human-validation records to retain their literal `VALIDATED` status and parses every mutation-matrix source and test anchor so a dead anchor cannot silently turn a dated audit green.

`TestExamProductTarget` recomputes the real scorecards for all three corpora rather than trusting the goldens. The strict target requires extraction precision and recall of at least 0.90. Perfect citation coverage and correctness. Zero identity, direction, third-party, closed, duplicate, and non-obligation leaks. Scorable owner, direction, due time, lifecycle, and closure linkage rows at or above 0.90. And every reported class recall at or above 0.90. Commitment identity and citation-role ratios become required at schema v3, where immutable message/block refs make them honestly measurable. Each schema-v1/v2 scorecard instead carries an explicit `LEGACY_UNSCORABLE` state and the reason that its frozen vault lacks those immutable refs; `commitmentID` must not fabricate evidence to make the ratio defined. Dedup ratio scope additionally requires at least one `duplicate_of` gold sample: the sealed v3 corpus has none, so its scorecards carry an explicit `NO_GOLD_SAMPLES_UNSCORABLE` state while `DupLeaks == 0` and `DedupCrossArtifact == 0` remain mandatory on every surface. The default CI row remains an honest, dated known-red pin tied to #138 until the final Gate 3 ratchet; #154's narrower daily-lane contract is green independently. The strict form, enabled with `MORA_EXAM_PRODUCT_TARGET=1`, stays visibly red in the manual mutation audit until the complete typed product contract exists and the measured target is actually met. Gate 1 therefore certifies that the exam is trustworthy. It does not claim that the product passes the exam.
...
`exam-render-v3.1` is schema-gated: schemas 1 and 2 retain their frozen renderer names and bytes. It amends v3 Gmail bodies so every message carries its own visible `From:` header; the ordered sender metadata was already present, but the original v3 body exposed only the first sender. No `obligations-v3` corpus is generated by machinery tests. Corpus authoring, prospective specification, and the human validation round are a separate sealed step. The v3 unit fixture lives only in `_test.go`, proving validator/lint/render behavior without creating or blessing evaluation ground truth.

The sealed `obligations-v3` corpus is the separately authored validation counterpart. Its contract changes only the source-structure paragraph needed to tell a reader that Gmail direction comes from each ordered message's sender and addressees, never from the thread-level address union. `TestExamCorpusV3` validates the ledger, runs all five lint families, renders twice, checks the pinned schema-v3 hash manifest and checked-out bytes, and refuses any hand-authored vault file; `TestExamCorpusV3RejectsTamper` proves a changed checked-out byte fails closed. Regeneration is explicit with `-updatev3`, the determinism guard covers the adapter, and `TestExamIntegrityExit` includes the v3 machine trust legs without claiming a completed reader-validation round.
...
**Scorer v2 measures Owner directly.** `Scorecard.scorer_version` freezes the contract at 2, and `Prediction.Owner` is compared with the ledger owner per class. `CitedBriefLine` now carries explicit Owner and Direction atoms from the materialized commitment. The adapter copies them rather than deriving either from `SectionKind == "open_loops"` (which would merely restate extraction precision). The scorer resolves the stable owner atom against the synthetic ledger identity map before comparing classes.

**The ratchet is now partly green, by measurement.** Meeting Owner, Direction, and DueTime are typed. The validated v2 meeting row scores all three at precision/recall 1.0 with `CriticalDirection == 0` and both direction classes exercised. The meeting adapter copies `CitedBriefLine.DueAt` directly: an explicit due date arrives as an ISO `YYYY-MM-DD` calendar day, otherwise it carries the `relative` or `none` sentinel. The frozen v1 gold includes a clock that is unrecoverable from its date-only opening text, so explicit-date comparison is intentionally day-grained. Clock-level due scoring waits for event-linked extraction to exist as a separate typed capability. Meeting Lifecycle and ClosureLinkage are also directly expressed with precision 1.0, and `ClosedLeaks` is zero on both frozen scorecards. Their recall remains intentionally partial on the v1/v2 corpora because those pre-PR1 Markdown fixtures do not preserve immutable message/block ids for every hidden inventory item. The adapter refuses to fabricate identity merely to improve a score. The daily CLI and MCP now copy Owner, Direction, DueTime, Lifecycle, and ClosureLinkage from the same materialized commitment generation onto `DigestItem`. All five daily rows are defined, precise, and non-vacuous on both frozen scorecards while citation coverage/correctness remain 1.0 and `Unmatched == 0`. Commitment identity, dedup, and citation roles remain fail-closed until their product lanes can be measured end to end. `Due` and `ClosureRef` carry explicit `none` sentinels for the gold values — without them, a surface that cannot express a field at all would score *correct* against an obligation whose gold value is none, and the row would be accidentally green. `Prediction.CommitmentID`, `DuplicateOf`, and typed citations (`opener`, `closure`, `supporting`) keep canonical selection, duplicate-copy citation retention, and closure evidence independently observable.
...
Counterparty is the one inventory metric that the schema-v3 contract defines independently of immutable commitment refs. An ID-bearing row keeps the authoritative commitment-ID join. An ID-less row can claim at most one still-unclaimed gold row with the same memory and resolved counterparty; ambiguity refuses credit, duplicate rows do not increase recall and still cost precision, and an ID-less row cannot claim gold reserved by an ID-bearing row. A present provider atom is authoritative. Only when the atom is absent may the scorer resolve the distinct `CounterpartyLabel` against a unique, case-insensitive ledger display or given name; the label never overrides an atom or creates identity.

### The metric sensitivity contract

The most dangerous eval failure is not a wrong number. It is a **green metric that is insensitive to the bug it claims to detect**. So `RequiredMetrics` registers every number on the `Scorecard` with a scorer-version and at least one red-team row that moves it, and three meta-tests make that structural: `TestEveryMetricHasASabotageCase` fails by metric name if a metric names no sabotage, `TestEveryRegisteredSabotageMovesItsMetric` executes each declared row against a typed positive control and fails unless that exact `Scorecard` field changes, and `TestMetricRegistryCoversEveryScorecardField` walks the struct by reflection so a new number cannot enter the scoreboard without declaring what breaks it. Scorer v2 adds named owner, due-value, commitment-id, duplicate-pointer, citation-role, inventory-lifecycle, and inventory-origin-escape mutations. DueTime is covered independently by a one-prediction flip to a wrong calendar day and by the due arm of `SurfaceClosedAsOpen`, whose invented day must reduce precision. `RecallUncapped` is driven by an explicit second prediction run in the empty-brief row. Leaving its zero value untouched is not movement.
...
The historical `TestDailyBriefHasNoObligationContract` row is now a green structural contract for [DIGEST-OBLIGATION-LANE #154](https://github.com/pyranthus-hq/mora/issues/154): `DigestItem` must retain owner, direction, due, lifecycle, and closure fields. `assertGate3DailyRatchet` separately requires both daily transports to express all four scored obligation dimensions precisely and non-vacuously, keep citation coverage/correctness at 1.0, and keep `Unmatched == 0`.
...
- **`internal/mora`'s `TestMain` disables the crash-durability fsyncs, and that costs no assertion.** `testmain_test.go` points `atomicio.MarkerSyncFn`/`atomicio.SyncDirFn` at no-ops unless `MORA_TEST_REAL_FSYNC=1`. WHY: on darwin `(*os.File).Sync` is `fcntl(F_FULLFSYNC)`, a real device barrier; one `mora init` issues 24 of them and ~651 tests call `run(t, "init")`. Measured, this is 87% of `init`'s cost (136 ms → 17.8 ms) and **3.74× on the whole non-race suite (359.7 s → 96.1 s), 1.86× under `-race` (534.3 s → 287.9 s)**. It is safe because an fsync is unobservable from userspace — which is exactly why the durability gates are CALL-TRACE assertions through these seams, not real crash tests. `withMarkerTrace` captures the *current* seam value and wraps it, so event ORDER is still recorded against a no-op inner function; tests that stub a barrier to an error assign their own function outright; and the real barriers keep production defaults in `internal/atomicio`'s own package tests. Verified load-bearing by mutation: deleting the barriers in `atomicio.WriteDurable` reddens `TestDurableMarkerFsyncsBeforeRename`/`TestDurableMarkerSyncsDirBeforeReturn`, and deleting the marker fsync at the `ingest_journal.go` call site reddens `TestKilledIngestRecovers/journal_header_synced`. **Each package extracted by the Stage B split (#320) needs its own such `TestMain` to keep the win** — `TestMain` is per-package.
