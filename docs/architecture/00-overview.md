...
| `internal/registry` | `internal/config`, `internal/memory` | Static connector descriptors and deterministic connector-instance identity: provider aliases, source/account keys, enabled-ingesting enumeration, display ordering/labels, upcoming capability, and platform filtering. Source persistence and connector dispatch remain in `internal/mora`. |
