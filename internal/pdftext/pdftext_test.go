...
var (
	pdfMaxFileSize int64 = DefaultMaxFileSize
	pdfMaxPages          = DefaultMaxPages
)

func extractPDFText(path string) (string, error) {
	return Extract(path, Options{MaxFileSize: pdfMaxFileSize, MaxPages: pdfMaxPages, MaxTextBytes: DefaultMaxTextBytes})
}
...
func TestDefaultOptionsAndMissingFile(t *testing.T) {
	opts := DefaultOptions()
	if opts.MaxFileSize != DefaultMaxFileSize || opts.MaxPages != DefaultMaxPages || opts.MaxTextBytes != DefaultMaxTextBytes {
		t.Fatal(opts)
	}
	if _, err := Extract(filepath.Join(t.TempDir(), "missing.pdf"), opts); err == nil {
