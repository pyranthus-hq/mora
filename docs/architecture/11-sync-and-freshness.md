...
| `internal/mora/mora.go` | 3791 | The wiring: `cmdSync` (`sync status` / `sync google` / `sync filesystem` / `sync imessage`), `ingestGoogle`/`ingestIMessage` (load→ingest→save status), `googleStatusPath`/`imessageStatusPath`, `sourceFreshness` (Phase-12 fix), `windowForSource`/`windowForIMessage`, `backfillEnabledGoogle`/`backfillEnabledFilesystem`/`backfillEnabledIMessage` (all now in `ingest.go`) + the `backfillGoogleFn`/`backfillIMessageFn` sync-first seams (Phase 13, `mora.go:1329-1332`), `cmdPulse`'s `--sync` flag, `installSchedule`/`schedulePlistFor`/`scheduleRunAtLoad` (launchd periodic re-pull, now in `schedule.go`). |
