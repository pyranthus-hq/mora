...
| `internal/meeting` | `internal/commitment`, `internal/governance`, `internal/graph`, `internal/health`, `internal/memory` | Validated cited meeting-brief DTOs/correction commands/historical rails plus deterministic next-event selection, sender-authored evidence text/historical framing, participant-roster gating, and pure meeting-evidence classification: notification/third-party/service exclusion, provider-specific ownership, actionable-question boundaries, unresolved/staleness/material-context policy. Mora supplies identity/config/service/time facts and retains attribution, governance decisions, vault/graph loading, ranking, budgeting, rendering, and CLI/MCP assembly. |
