...
| `internal/meeting` | `internal/commitment`, `internal/graph`, `internal/memory` | Deterministic next-event selection, sender-authored evidence text/historical framing, participant-roster gating, and pure meeting-evidence classification: notification/third-party/service exclusion, provider-specific ownership, actionable-question boundaries, unresolved/staleness/material-context policy. Mora supplies identity/config/service/time facts and retains attribution, commitments, governance, vault/graph loading, ranking, budgeting, and CLI/MCP assembly. |
