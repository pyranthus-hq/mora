...
| `internal/commitment` | `internal/evidence`, `internal/evidencetext`, `internal/identity`, `internal/memory` | Typed derived commitment record/span/citation JSON model and canonical record/opener-citation construction plus provider-neutral speech ownership/manual-promise adjudication plus shared obligation direction/state/due vocabulary, deadline classification, stable evidence identity, deterministic guarded lifecycle transitions with typed closure citations, ambiguity gaps, citation union, stable-ID uniqueness, and provenance-gated duplicate/canonical projections. `internal/commitmentclassify` consumes these policies; Mora retains health-aware materialization, SQLite generation ownership, and governance application. |
| `internal/commitmentclassify` | `internal/commitment`, `internal/evidence`, `internal/evidencetext`, `internal/graph`, `internal/identity`, `internal/imessage`, `internal/meeting`, `internal/memory` | Deterministic provider-aware classification engine: manual promises, Gmail messages/subjects/quoted fulfillment, iMessage structured/legacy turns, reported actors, acceptance collapse, third-party exclusion, exact record construction, and fail-closed evidence admission. Mora supplies config-derived self/service facts and retains materialization, transaction, health, and governance authority. |
| `internal/contextintent` | `internal/commitment`, `internal/genericutil`, `internal/memory`, `internal/recency` | Deterministic context-query intent recognition, qualifier extraction, project/service ranking, canonical-open commitment selection and ordering, and budgeted typed-context rendering. Mora retains index/vault loading, search-filter authority, retrieval routing, clocks, and MCP assembly. |
...
| `internal/previewfilter` | `internal/graph`, `internal/memory` | Pure entity/scope/since-days preview filtering, alias membership sets, negative-window clamping, fail-closed timestamp matching, and deterministic caller-owned-DB entity resolution with strict ambiguity reporting. Mora retains index DB lifecycle, user-facing ambiguity/busy errors, option parsing, and digest/MCP orchestration. |
...
| `internal/evidence` | `internal/memory` | Provider-neutral, strict cited-evidence identity/JSON codec and deterministic memory/provider/source/UTC derivation shared by meeting briefs and commitment materialization. |
| `internal/evidencetext` | Standard library | Provider-neutral evidence text normalization: authored-body boundaries, quote/signature stripping, sentence segmentation, historical framing, URL/noise/trivia filtering, and phrase boundaries. |
