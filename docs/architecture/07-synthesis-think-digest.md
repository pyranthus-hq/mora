...
- **Token budget is clamped before the `×4` multiply.** `resolveContextBudget` clamps to `maxContextTokens` *then* multiplies (`mcp.go`); reordering risks int overflow on a hostile `max_tokens`. Also: `mora context --budget` is **chars**, `context_memory max_tokens` is **tokens** — different units, different paths.
