...
const (
	...
	DecisionLocalAhead Decision = "local-ahead"
)

type Options struct {
	CurrentVersion, AppRoot         string
	CheckOnly                       bool
	Token                           string
	Stdout                          io.Writer
...
func Run(ctx context.Context, opts Options) error {
	...
	candidate, found, err := detectLatestAppReleaseForRepo(ctx, source, arch, opts.RepoOwner, opts.RepoName)
	if err != nil {
	...
	verdict, isLocalBuild, err := opts.Decide(current, candidate.version)
	if err != nil {
		return err
	}
	switch verdict {
	case DecisionLocalAhead:
...
func detectLatestAppRelease(ctx context.Context, source selfupdate.Source, arch string) (appReleaseCandidate, bool, error) {
	return detectLatestAppReleaseForRepo(ctx, source, arch, "pyranthus-hq", "mora")
}

func detectLatestAppReleaseForRepo(ctx context.Context, source selfupdate.Source, arch, owner, repo string) (appReleaseCandidate, bool, error) {
	releases, err := source.ListReleases(ctx, selfupdate.NewRepositorySlug(owner, repo))
	if err != nil {
