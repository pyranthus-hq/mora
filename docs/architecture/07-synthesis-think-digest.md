...
After window/delta selection, `attachDigestCommitments` reads the generation-stamped whole-vault commitment snapshot and copies `Owner`, `Direction`, `DueAt`, `Lifecycle`, and `ClosureRef` onto each surfaced `DigestItem`. It never re-runs `classifyCommitment` over the tail-clipped digest snippet: meeting and daily therefore consume the same materialized generation. Because the daily surface is artifact-grained and the payload has one obligation lane, a memory with multiple independently anchored commitments is left untyped rather than guessed.
