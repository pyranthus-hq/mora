...
Mora turns a query string into a ranked list of memories. FTS5/BM25 gives the
exact-match base. Mora has two embedders: a fixed static-hash floor and an
optional Ollama semantic model. It can add a 1-hop graph search and join results
with RRF. A router selects static keyword search (parent FTS plus Gmail
message-segment FTS) or hybrid search from measured recall.

## Files

| File | Lines | Responsibility |
|---|---|---|
| `internal/mora/hybrid.go` | 382 | The four-arm hybrid engine: `hybridSearch`/`hybridSearchTrace`, the `defaultSearch` router + `embedderIsSemantic` gate, parent FTS (`ftsSearchIDs`), vector (`vectorSearchIDs`), graph (`graphExpandIDs`), Gmail segment retrieval, `rrf` fusion, `vectorsAvailable`, the query-time gazetteer loader, the `retrievalTrace` attribution seam, pool sizing |
...
There are **two** search paths. The choice between them is this subsystem's main
design rule. `defaultSearch` (`hybrid.go:70`) supports both `mora search` (CLI,
`mora.go:497`) and the `search_memory` MCP tool (`mora.go:2979`):
...
**Why gate at all — the empirical regression.** The T2 recall eval (2026-06-06) measured, on Adit's real-query golden set, that hybrid retrieval beat the then-parent-FTS-only baseline **only when a real semantic embedder was active**. Under the static-hash floor, hybrid *regressed* recall: **0.591 → 0.394 @5** (cited in the `embedderIsSemantic` doc comment, `hybrid.go`). The static-hash vector arm is lexical-feature cosine, not prose semantics, so it injects ranking noise that drags the BM25 anchor down. The current router therefore sends static-hash searches to the **static keyword surface** (`searchMemories`: parent FTS plus Gmail/iMessage segment FTS) and reserves vector/graph hybrid search for the case where it was proven to help.

**Why gate on `chooseEmbedder()`, not just `MORA_EMBEDDER`** (`hybrid.go`): a vector-empty hybrid is **not** equivalent to the static keyword surface. The graph-expansion arm still feeds IDs into RRF and shifts the ranking, so a hybrid call with `MORA_EMBEDDER=ollama` but the daemon **down** would not reproduce the measured baseline. `chooseEmbedder` resolves the *actually active* embedder (Ollama opted-in **and** reachable), so "Ollama configured but daemon dead" correctly degrades to static parent+segment retrieval. (This was a Codex-review catch, noted in the comment.)
...
**`context_memory` / `think` do not gate** — they call `hybridSearch` unconditionally (`mora.go:551` for the `mora context` CLI, `mora.go:2997` for the MCP `context_memory`). They are still safe under static-hash because `hybridSearchTrace` now **drops the vector arm entirely when the embedder is not semantic** (`useVec = embedderIsSemantic(emb)`, `hybrid.go`): the static-hash vectors are deterministic noise, and fusing a noise arm via RRF *demotes* strong FTS hits (the FUSION-dominant regression where hybrid scored below the then-parent-FTS-only baseline on the live recall eval). Under static-hash these surfaces therefore fuse parent FTS + graph + Gmail segments. The vector arm joins only when Ollama is active. See `docs/design/2026-06-10-retrieval-ranking.md`.

```mermaid
flowchart TD
    Q["query string"] --> R{"defaultSearch:<br/>embedderIsSemantic(chooseEmbedder())?"}
    R -->|"static-hash<br/>(default, or Ollama daemon down)"| F["searchMemories<br/>parent FTS + Gmail segments<br/>RRF only when segment arm participates"]
    R -->|"Ollama opted-in AND reachable<br/>(semantic)"| H["hybridSearch<br/>4 arms → RRF<br/>(hybrid.go)"]
    F --> OUT["ranked []Memory"]
    H --> OUT
...
The index is an FTS5 virtual table `memories_fts(id, scope, title, tags, source, text)` (`mora.go:2037`) over the projected `memories` table. Both the static path's parent arm (`searchMemories`, `search.go`) and the hybrid FTS arm (`ftsSearchIDs`, `hybrid.go`) `MATCH` against it and order by the BM25 score then `m.id` — `searchMemories` writes it as `ORDER BY score, m.id` where `score` aliases `bm25(memories_fts)` (`search.go`), and `ftsSearchIDs` writes `ORDER BY bm25(memories_fts), m.id` (`hybrid.go`). The secondary `m.id` sort is **load-bearing for determinism**: BM25 alone leaves equal-score rows in undefined order, which would jitter the pool boundary run-to-run.

**Trusted-source / time-window predicates (issue #241).** `memories` carries three additional indexed columns introduced by D's predecessor v4 and included in canonical combined schema v5: `provider` (the connector family, canonicalized via `providerToType` at write time — `index.go`/`index_upsert.go`/`share_gen.go`), `account` (the raw account label, e.g. `"work"`), and `created_at_unix` (`CreatedAt` parsed once, at write time, as an RFC3339 instant — `createdAtUnix`, `search_filters.go`; a value that fails to parse is stamped `math.MinInt64`, NOT `0`/epoch — `0` looks like a safe "always in the past" choice, but `sqlPredicate`'s cutoff is `now - since_hours`, and at a large enough `since_hours` (up to `maxSinceHours`, ~292 years) that cutoff itself goes negative, before 1970; a row stamped `0` would then satisfy `created_at_unix >= cutoff` and leak through the exact filter meant to exclude it. `math.MinInt64` is below any cutoff the bounded arithmetic can ever produce, so a malformed timestamp fails closed regardless of window size). When `search_memory`/`context_memory` supply an optional `source`/`since_hours` filter (`searchFilters`, `search_filters.go`), every arm — `searchMemories`, `ftsSearchIDs`, `vectorSearchIDs`, `graphExpandIDs`, `gmailSegmentQueryArm`, and `share.go`'s `searchShareIndex` for a subscribed corpus — appends the SAME predicate (`searchFilters.sqlPredicate`: `AND provider = ?` [`AND account = ?`] [`AND created_at_unix >= ?`]) to its query **before** `ORDER BY`/`LIMIT`, so a filtered-out row is never fetched, never ranked, and can never crowd a matching row out of the pool. The vector arm applies it on the same `SELECT` that feeds the cosine-similarity loop, so a filtered-out row never earns a cosine score in the first place. The segment arm applies it to the joined parent `memories` row before segment BM25 ordering, so an excluded Gmail message cannot earn a segment rank or be hydrated back into the result set. See [MCP server](./06-mcp-server.md)'s "Trusted-source / time-window filters" section for the full caller-facing contract (grammar, fail-closed cases, the `filters`/`excluded_by_filter` response shape, and why `source="filesystem"` is rejected rather than silently unmatchable).
...
`vectorsAvailable` (`hybrid.go`) checks the table exists **and is populated**, so a pre-I2 index degrades gracefully to parent FTS plus Gmail segments.
...
`gmailSegmentQueryArm` (`internal/mora/gmail_segments_search.go`) is a fourth candidate source, present in **both** `searchMemories` (static parent+segment) and `hybridSearchTrace` (hybrid): it runs the SAME query against `gmail_segments_fts` (the per-*message* derived projection — see [data model](./01-data-model-and-storage.md)) and maps hits to **parent** thread memory ids before fusion/slot accounting. A parent-aware SQL window first selects each thread's strongest matching segment (best FTS5 hidden `rank`, the same order as `bm25(gmail_segments_fts)`, then lowest `evidence_ref`), orders those winners by score, `evidence_ref`, then `memory_id`, and only then applies the same bounded parent pool as the other production arms. A thread with many strong messages therefore contributes exactly one candidate and cannot consume a raw segment-row LIMIT or make the parent/evidence maps unbounded. This is what proves a rare term buried in one short message of a long, diluted thread can still win — parent-grain FTS scores the *whole* joined body, so a long thread loses BM25's length normalization to short single-occurrence decoys; the segment arm scores that one message's *own* short text instead.

In `hybridSearchTrace`, the segment arm is a fourth list passed to `rrfWeighted([...], weights, fp.k)`. Its weight is `gmailSegmentArmWeight = 1.0`, alongside the FTS, vector, and graph arms. It is also stored in `retrievalTrace.Segment`. A segment hit is a text hit, not a graph-only association. It can help the separate confidence check only when one returned row contains every meaningful query term. Different rows cannot pool partial matches. `searchMemories` uses `admitGmailSegmentCandidates` to load segment matches whose parent missed the wider parent FTS pool. `fuseGmailSegmentArm` then runs a two-arm RRF: parent weight `1.5`, segment weight `1.0`, and `k = 10`. The two lists stay separate, so a segment-only row never gets a fake parent rank. If there are no segment matches, this path changes nothing. If the segment arm participates, scores move to the positive RRF scale. Confidence then reports `scale="rrf_fused"` and uses gap-based strength plus the exact-word check.

Bounding candidate parents does not weaken DQ5's match-based receipt rule. After final slot accounting, `completeGmailSegmentEvidence` performs a separate winner lookup only for missing Gmail ids in the bounded final result, chunked to keep SQLite bind counts bounded. Thus a parent returned by another arm still receives its strongest matching segment even when its segment rank fell below the candidate pool. Deep-trace evidence is discarded and never participates in production receipts.
...
`rrfWeighted` (`hybrid.go`) is **weighted rank-based Reciprocal Rank Fusion**: `score(id) = Σ wᵢ/(k + rank+1)` across four arm positions (parent FTS, vector, graph, Gmail segment). An unavailable or non-participating arm contributes an empty list, so static-hash omits vector and a query with no segment hits receives no contribution from that position. Rank-based fusion is deliberate — it combines both BM25 rankings and cosine's `[0,1]` scores **without normalization**. The weights + damping live in `defaultFusion = {fts:1.5, vec:1, graph:1, k:10}`, with `gmailSegmentArmWeight=1.0` appended (overridable per-`Config` via `fusionOv` for the `TestEvalWeightSweep` tuning grid). **The damping `k` is the load-bearing knob, not the weights:** the textbook `k=60` is far too flat for a ~50-doc pool — a rank-50 also-ran contributes `1/110`, nearly the `1/61` of a rank-0 hit — so weak cross-arm agreement demoted strong FTS hits. Dropping to `k=10` sharpens the head and migrated FUSION→HIT. A gentle `fts=1.5` anchors the exact-match arm (heavier FTS regressed — it buries the vector arm's vocabulary-mismatch rescues). `rrf` remains as the equal-weight wrapper. Tuned on the live golden set. See `docs/design/2026-06-10-retrieval-ranking.md`.

**Pool sizing** (`hybrid.go`): each established arm is queried at `pool = limit * 5`, floored at 50. The segment candidate query returns at most `pool` distinct parent memories, selecting one winning segment per parent before its SQL `LIMIT`; only those bounded parents can enter the candidate union or fusion. After slot accounting, separate final-result evidence completion looks up only missing Gmail ids from the bounded result, in fixed-size chunks, so a parent admitted by another arm can still receive its strongest matching segment without retaining an unbounded arm list. The graph arm's per-person LIMIT is `pool`, but its deduped union across people may *exceed* pool and is fused whole. Capping it would change the fused ranking for multi-person queries and break byte-identity.
...
- **Only the MCP surface snippets.** The CLI `mora search` and `mora list` paths keep full bodies + meta. `mcpSearchMemory` applies `snippetMemories` after `defaultSearch`; `mcpListMemory` applies it after `listMemories`, then both handlers use `budgetSearchResults` to cap the aggregate on whole-memory boundaries. Search reserves room inside the same ceiling for evidence-manifest and ranking receipts; provenance is never appended after budgeting without a bound.
...
Both search paths run one more pass, post-fusion/pre-truncate, before the `limit`-sized result is handed back: `clusterAndTruncate` (`cluster.go`) collapses records that describe the same real-world event into one result slot. `searchMemories` widens its SQL `LIMIT` to a deeper candidate pool (`limit*5`, floor 50 — the same formula `hybridSearchTrace` already used for its arm pool) so a cluster's freed slots have real distinct candidates to backfill from, then clusters and truncates to `limit`; `hybridSearchTrace` clusters the fused, pre-truncate `ids` list at the exact point it used to just slice `ids[:limit]`. Three cheap, OR'd anchor rules decide a link: identical non-empty `(Provider, ProviderID)`; identical non-empty normalized `ContentHash` (the rolling-digest and exact-copy rule); or explicit person/entity overlap within a strict 24-hour `meta.occurred_at` window. The person/time rule never falls back to `CreatedAt`, and structural notification labels never count as identities. Clusters form greedily against the strongest head, not through transitive closure, and a candidate exceeding five total members is refused whole. This keeps fan-out from becoming a fabricated single event while giving digest-derived copies one canonical observation with compact lineage. See `internal/mora/cluster_contract_test.go` and `internal/search/cluster_test.go`.
...
## Evidence manifests, ranking receipts, and attribution

`search_memory` returns an `evidence_manifest` whose stable IDs close over every top-level result, folded corroborating ref, and message-segment citation in that response. Each entry carries a canonical source instance, timestamp, and a deep link when the provider supplies or safely derives one. A sibling `ranking` array records final position, score, contributing retrieval lanes, the ranking method, and absorbed evidence IDs. Existing `health`, `shares_unhealthy`, and `excluded_by_filter` fields identify unavailable or deliberately excluded sources without interpreting prose errors.

Before the MCP byte cut, `diversifyEvidence` pins the strongest result and greedily promotes unseen source, entity, month, and evidence type. It is a permutation, not deletion: repetition remains in the response when it fits and exact/event duplicates remain accessible through lineage. `context_memory` publishes the manifest beside its assembled text, and `think` evidence carries the same canonical source, timestamp, and optional deep link directly.

iMessage evidence keeps authorship (`sender`), direction (`incoming`/`outgoing`), and audience (`direct`/`group`) separate. A group segment has no invented direct recipient: ambient participants remain conversation context, while only the provider-observed sender owns the message. Gmail continues to preserve explicit To/Cc recipients and segment authors independently.
...
- **Search routes via `defaultSearch`, gated on the *actually active* embedder.** Static parent+segment retrieval under static-hash (including Ollama-opted-but-down). Hybrid only when `chooseEmbedder()` returns a semantic embedder. **Why:** hybrid regressed recall 0.591→0.394 under static-hash against the measured parent-FTS baseline, and a vector-empty hybrid is not equivalent to the static surface because the graph arm still perturbs RRF.
