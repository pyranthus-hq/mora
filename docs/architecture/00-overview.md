...
| `internal/mcp` | `internal/atomicio`, `internal/config` | MCP protocol, budgets, mutation authority, and pending-proposal storage: framing/results, schemas/arguments, context ceilings, policy-consistent instructions/dispatch decisions, validated IDs/paths, exact atomic JSON, integrity checks, and deterministic listing. Proposal validation/approval execution, handlers, health/usage, and orchestration remain in `internal/mora`. |
