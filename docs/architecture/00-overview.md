...
| `internal/ingest` | `internal/atomicio`, `internal/config`, `internal/memory` | Connector-ingest journal and lease lifecycle: mark-before-visible header publication, best-effort path records, status, PID ownership/reclamation, and post-rebuild recovery/compaction with live-lease protection. Stage B extraction; connector dispatch and registry wiring remain in `internal/mora`. |
| `internal/registry` | `internal/atomicio`, `internal/config`, `internal/genericutil`, `internal/memory` | Static connector descriptors/identity plus atomic `sources.json` persistence: provider aliases, instance keys, consent normalization, enabled-ingesting enumeration, display metadata, upcoming capability, and platform filtering. Source mutations, CLI, and connector dispatch remain in `internal/mora`. |
