package mora

import "testing"

// embedderIsSemantic is the routing gate behind search_memory and `mora search`.
// The T2 recall eval proved hybrid beat the then-parent-FTS baseline ONLY with a
// real semantic embedder, so the default path must stay on static keyword search
// (parent FTS + Gmail segments) under the static-hash floor and switch to hybrid
// only when a semantic embedder is actually active — otherwise recall regresses
// (0.591 -> 0.394 @5).
func TestEmbedderIsSemantic(t *testing.T) {
	if embedderIsSemantic(defaultEmbedder()) {
		t.Fatal("static-hash floor must be classified non-semantic (default search stays on static parent+segment retrieval)")
	}
	// ollamaEmbedder.ModelID() is "ollama:<model>" and does not probe the daemon,
	// so this is deterministic and CI-safe without a running Ollama.
	if !embedderIsSemantic(ollamaEmbedder{model: "nomic-embed-text"}) {
		t.Fatal("an Ollama embedder must be classified semantic (default search uses hybrid)")
	}
}
