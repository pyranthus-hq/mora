...
| `internal/search` | (leaf — no internal deps) | Safe FTS5 MATCH compilation and CLI search-argument parsing, including case-aware stopword handling and deterministic OR-token ordering. Stage B extraction; retrieval execution, fusion, filtering, snippets, and context assembly remain in `internal/mora`. |
...
| `internal/memory` | (leaf — no internal deps, no `net/*`) | Shared connector seam: `Item`, `Fetcher`, `Ingest`, `MapItem`, `MappedMemory`, `SyncStatus`, `StableID`/`ContentHash`/`SafeFilename`. The contract connector packages implement. |
...
| `internal/index` | `internal/config` | Embedded SQLite index storage boundary: canonical database path, fail-closed read-only URI, serialized WAL-writer DSN, schema fencing/readiness probes, deterministic vault-manifest construction, and best-effort failure stamping. Stage B extraction; rebuild, auto-heal policy, graph/vector construction, and CLI wiring remain in `internal/mora`. |
...
| `internal/mora` | all connector packages + `internal/atomicio`, `internal/config`, `internal/doctor`, `internal/genericutil`, `internal/storage`, `internal/schedule`, `internal/tasks`, `internal/usage` + lipgloss, go-isatty, go-selfupdate | The hub: CLI dispatch, connector wiring, filesystem ingestion, Markdown render/parse, SQLite index and search, derived entity graph, synthesis, MCP server, doctor, scheduler, eval harness, and self-update. |
