...
func GenerateCask(tag string, checksums io.Reader, autoUpdates bool) ([]byte, error) {
	...
	fmt.Fprintln(&out)
	fmt.Fprintln(&out, `  depends_on :macos`)
	fmt.Fprintln(&out)
	fmt.Fprintln(&out, `  app "Mora.app"`)
	fmt.Fprintln(&out, `  binary "#{appdir}/Mora.app/Contents/MacOS/mora", target: "mora"`)
	fmt.Fprintln(&out)
	fmt.Fprintln(&out, `  # Third-party tap: literal preflight_steps cannot check a conflicting user app.`)
	fmt.Fprintln(&out, `  # rubocop:disable Cask/InstallSteps`)
	...
	fmt.Fprintln(&out, `  end`)
	fmt.Fprintln(&out)
	fmt.Fprintln(&out, `  # rubocop:enable Cask/InstallSteps`)
	fmt.Fprintln(&out)
	...
	fmt.Fprintln(&out, `    Installation does not enable a schedule or configure connectors.`)
