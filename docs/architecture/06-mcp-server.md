...
| `internal/health/compact.go`; `internal/mora/health_envelope.go` | `health.Compact`, `health.ProjectCompact`, `health.FromParts`; thin fact-loading and CLI-render adapters | The lower package owns the bounded deterministic health projection every MCP tool/HTTP route carries; Mora loads the arms and prints the CLI banner |
